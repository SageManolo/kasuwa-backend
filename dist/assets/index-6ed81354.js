...
// background:red;
`;function j0(){const[e,t]=ve.useState([]),[n,r]=ve.useState({});function i(){fetch("https://kasuwaapp7.onrender.com/public").then(o=>o.json()).then(o=>{let l=o[0].commodities,a=o[1].commodities;const u=(w,x)=>((w-x)/x*100).toFixed(2);let c=[],g=[];for(let w=0;w<l.length;w++){let x=!1;for(let S=0;S<a.length;S++)if(l[w].name==a[S].name){if(x)break;c[a[S].name]=u(l[w].price,a[S].price),g[a[S].name]=l[w].price,x=!0}}console.log("Array3:",c),t(g),console.log("arr4",g),console.log("c0mmodity:",n);let m=l.map(w=>w.price),p=a.map(w=>w.price);console.log(m),console.log(p),r(c)}).catch(o=>{console.log(o)})}return ve.useEffect(()=>{i()},[]),h(F0,{commodities:e,commodityPercentage:n,children:Object.keys(n).map(o=>h(M0,{name:o,price:e[o],percentage:n[o]}))})}let D0=Um`
    0% {
